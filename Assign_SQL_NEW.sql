...
select p.product_id, p.product_name, p.price, s.customer_id, s.product_id, c.first_name, c.last_name, c.customer_id
from customers c
...
select s.total_amount, s.product_id, p.product_name, p.product_id
from sales s
join products p on s.product_id = p.product_id;
-- 15. Write a query to join the `Customers` and `Sales` tables and find the total amount spent by each customer.
select c.customer_id, c.first_name, c.last_name, s.customer_id, s.total_amount
from customers c 
join sales s on c.customer_id = s.customer_id;
...
select c.customer_id, c.first_name, c.last_name, p.product_name, p.product_id, s.quantity_sold, s.customer_id
from customers c
join sales s on c.customer_id = s.customer_id
join products p on p.product_id = s.product_id;
...
from customers c1 join customers c2 on c1.membership_status = c2.membership_status
and c1.customer_id < c2.customer_id
order by c1.membership_status, c1.customer_id, c2.customer_id;
		
-- 18. Write a query to join the `Sales` and `Products` tables, and calculate the total number of sales for each product.
select p.product_name, s.product_id, s.quantity_sold, count(s.sale_id) as total_number_of_sales
from products p
...
select products from products where stock_quantity < 10; 
...
select c.customer_id, c.first_name, c.last_name, p.product_id, p.product_name, p.category
...
select c.customer_id, c.first_name, c.last_name, sum(distinct s.total_amount) as total_sales
from customers c join sales s on c.customer_id = s.customer_id
where sale_date >= '2023-01-01' and sale_date <= '2023-12-31'
group by c.customer_id, c.first_name, c.last_name
order by total_sales desc ; 
-- 25. Write a query to join the `Products` and `Sales` tables and select the most expensive product sold.
select p.product_id, p.product_name, max(p.price) as most_expensive_product
...
select sum(distinct s.sale_id ) as total_number_of_customers
...
select c.customer_id, c.first_name, c.last_name, count (distinct s.product_id) as total_product_purchased,
...
select p.product_id, p.product_name,  avg(s.quantity_sold) as average_products_sold
...
select * from sales;
select p.product_id, P.product_name, P.category, count(distinct sale_id) as total_sales
from products p join sales s on p.product_id = s.product_id
...
select c.customer_id, c.first_name, c.last_name, s.quantity_sold, sum(s.total_amount) as Total_amount_spent
from customers c join sales s on c.customer_id = s.customer_id 
where sale_date >= '2023-01-01' and sale_date <= '2023-12-31'
...
select p.product_id, p.product_name, s.quantity_sold, sum(s.total_amount) as total_sales
from products p join sales s on p.product_id = s.product_id
...
select 
c.customer_id, c.first_name, c.last_name, c.registration_date, s.sale_id,
s.sale_date, s.sale_date - c.registration_date
from customers c
join sales s
on c.customer_id = s.customer_id
where s.sale_date - c.registration_date <= 7 and
s.sale_date - c.registration_date >= 0
...
select c.customer_id, c.first_name, c.last_name, p.product_id, p.product_name, sum(s.quantity_sold) as total_quantity_sold
from products p join sales s on p.product_id = s.product_id
...
select p.product_id, p.product_name, p.category, p.price, p.supplier, p.stock_quantity
from products p 
where p.stock_quantity = (select min(stock_quantity) from products)
	or p.stock_quantity = (select max (stock_quantity) from products);
...
select c.customer_id, c.first_name, c.last_name, c.membership_status, p.product_name, sum(s.total_amount) as total_sales_amount
from customers c join sales s on c.customer_id = s.customer_id
join products p on p.product_id = s.product_id
where c.membership_status = 'Gold'
group by c.customer_id, c.first_name, c.last_name, c.membership_status, p.product_name;
...
select s.product_id, p.product_name,i.stock_quantity
from sales s left join inventory i on s.product_id = i.product_id
join products p on s.product_id= p.product_id
where i.stock_quantity != 0; 
-- 45. Write a query to find the top 5 customers who have made the highest purchases.
select c.customer_id, c.first_name, c.last_name, count(distinct s.sale_id) as total_number_of_purchases
from customers c join sales s on c.customer_id = s.customer_id
group by c.customer_id, c.first_name, c.last_name
order by total_number_of_purchases desc limit 5;
-- 46. Write a query to calculate the total number of unique products sold in 2023.
select p.product_id, p.product_name, count(distinct s.product_id) as Unique_products_sold
from products p join sales s on p.product_id = s.product_id
where sale_date >= '2023-01-01' and sale_date <= '2023-12-31'
group by p.product_id, p.product_name;
...
from products p 
where p.product_id not in (select distinct s.product_id from sales s 
where s.sale_date >= date '2024-02-05' - interval '6 month');
...
from products p join sales s on p.product_id = s.product_id
group by p.product_id, p.product_name
having p.price between 200 and 800;
